...
class KeywordsEntryPoint(BaseEntryPoint):
    ...
    def execute(self, argv):
        ...
        parser.add_argument(
            "--all",
            dest="use_all_records",
            action="store_true",
            help="Select all records from the dataset for keyword extraction",
        )
        ...
        parser.add_argument(
            ...
            dest="use_all_records",
            action="store_false",
            help="Select only included records from the dataset for keyword extraction",
        )
        ...
        if len(args.asreview_files) > 1:
            raise ValueError(
                "Extracting keywords from multiple project files"
