...
def extract_keywords(asreview_filename, outputfile_name, method):
    ...
    dataset_w_labels = dataset.df.join(state_df.set_index('record_id')[['current_label']], on='record_id')
    
   # combine title and abstract into text
    dataset_w_labels["text"] = dataset.title + " " + dataset.abstract
    dataset_included = dataset_w_labels[dataset_w_labels['current_label'] == 1].copy()
     # Applying preprocessing to the dataset
    dataset_included["text"] = dataset_included["text"].apply(pre_process)
    dataset_included.drop(['current_label'], axis=1, inplace=True)
    ...
    if method == "tf-idf":
        ...
        raise ValueError("This keyword extraction method is not implemented. Please select 'tf-idf'.")
